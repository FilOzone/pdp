...
library MerkleVerify {
     ...
     */
    function verify(bytes32[] memory proof, bytes32 root, bytes32 leaf, uint256 position, uint256 treeHeight)
        internal
        view
        returns (bool)
    {
     ...
     * This version handles proofs in memory.
     */
    function processInclusionProofMemory(bytes32[] memory proof, bytes32 leaf, uint256 position)
        internal
        view
        returns (bytes32)
    {
...
library Hashes {
    ...
    function _efficientSHA254(bytes32 a, bytes32 b) private view returns (bytes32 value) {
        assembly ("memory-safe") {
            ...
            if iszero(staticcall(gas(), 0x2, 0x00, 0x40, 0x00, 0x20)) { revert(0, 0) }
