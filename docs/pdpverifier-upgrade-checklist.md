...
- `PDPVerifier upgrade from \`vA.B.C\` to \`vX.Y.Z\``
...
Create one tracking issue from the generic PDPVerifier upgrade template.

Use it to track the full rollout:
- Calibration first
- Mainnet second
...
- release-closeout follow-ups

Mainnet should not start until Calibration finishes successfully.
...
Record that commit hash in the upgrade issue.
