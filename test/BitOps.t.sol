...
contract BitOpsTest is Test {
    ...
    function testClzPowersOfTwo() public pure {
        for (uint16 i = 0; i < 256; i++) {
            ...
            assertEq(
                result,
                255 - i,
                string(abi.encodePacked("CLZ of 2^", vm.toString(i), " should be ", vm.toString(255 - i)))
            );
        }
    }

    function testClzSelectValues() public pure {
    ...
    function testCtzZero() public pure {
        uint256 result = BitOps.ctz(0);
        assertEq(result, 256, "CTZ of 0 should be 256");
    }

    function testCtz1LShift255() public pure {
    ...
    function testCtzSelectValues() public pure {
