...
contract MerkleProveTest is Test {
    ...
    function testVerifyTreeTwoLeaves() public view {
        ...
        for (uint256 i = 0; i < leaves.length; i++) {
            ...
            assertTrue(
                MerkleVerify.verify(proof, root, leaves[i], i, tree.length),
                string.concat("Invalid proof ", vm.toString(i))
            );
            assertFalse(
                MerkleVerify.verify(proof, root, leaves[i], i + 1, tree.length),
                string.concat("False proof ", vm.toString(i))
            );
        }
    }
    ...
    function testVerifyTreeThreeLeaves() public view {
        ...
        for (uint256 i = 0; i < leaves.length; i++) {
            ...
            for (uint256 j = 0; j < leaves.length; j++) {
                if (j != i) {
                    assertFalse(
                        MerkleVerify.verify(proof, root, leaves[i], j, tree.length),
                        string.concat("False proof ", vm.toString(i))
                    );
                }
            }
    ...
    function testZeroRootsComputed() public view {
        ...
        // }
        for (uint256 height = 0; height <= 50; height++) {
    ...
    function printTree(bytes32[][] memory tree) internal pure {
        ...
        for (uint256 i = 0; i < tree.length; i++) {
            console.log("Level ", i, ":");
            for (uint256 j = 0; j < tree[i].length; j++) {
